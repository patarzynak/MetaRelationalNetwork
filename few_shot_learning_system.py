...
from meta_neural_network_architectures import RN, CNN_MLP
from meta_neural_network_architectures_other import CNNMLPNetwork
...
class OurMAMLFewShotClassifier(nn.Module):
    def __init__(self, im_shape, device, args):
        ...
        """
        super(OurMAMLFewShotClassifier, self).__init__()
        self.args = args
        self.device = device
        ...
        # self.classifier = CNNMLPNetwork(im_shape=self.im_shape, num_output_classes=self.args.
        #                                       num_classes_per_set, question_length=33,
        #                                       args=args, device=device, meta_classifier=True).to(device=self.device)
        #self.classifier = CNN_MLP(args=args, device=device).to(device=self.device)
        self.classifier = CNN_MLP(args=args, device=device).to(device=self.device)
    ...
    def apply_inner_loop_update(self, loss, names_weights_copy, use_second_order, current_step_idx):
        ...
        #print("GRRRAADDSSS", grads)
        #print(len(grads))
        #for i in range(0,31):
        #print(16, grads[16].size())
        #print(grads[0][0][0], grads[16][0][0], grads[28][0][0])
        #print("weights", names_weights_copy)
    ...
    def get_across_task_loss_metrics(self, total_losses, total_accuracies, names_weights_copy):
        ...
        #with open('almost_loss_acc.txt', 'a') as the_file:
        #    the_file.write('{};{}\n'.format(losses['loss'], losses['accuracy']))
    ...
    def forward(self, data_batch, epoch, use_second_order, use_multi_step_loss_optimization, num_steps, training_phase):
        ...
        """
        question_set, x_support_set, x_target_set, y_support_set, y_target_set = data_batch
        ...
        for task_id, (question_set_task, x_support_set_task, y_support_set_task, x_target_set_task, y_target_set_task) in \
                enumerate(zip(question_set,
                              x_support_set,
                              y_support_set,
                              x_target_set,
                              y_target_set)):
            ...
            names_weights_copy = self.get_inner_loop_parameter_dict(self.classifier.named_parameters())
             ...
             # EDITED
            n, h, w, c = x_target_set_task.shape

            # Channel first
            x_support_set_task = x_support_set_task.permute(0, 1, 4, 2, 3)
            x_target_set_task = x_target_set_task.permute(0, 3, 1, 2)
    ...
    def net_forward(self, x, y, question, weights, backup_running_statistics, training, num_step):
        """
        ...
        """
        preds = self.classifier.forward(x=x, question=question.float(), params=weights,
                                        training=training,
                                        backup_running_statistics=backup_running_statistics, num_step=num_step)
        
        loss = F.cross_entropy(input=preds, target=y)
        #loss = F.nll_loss(input=preds, target=y)
        #print("predictions", preds, " target ", y, " loss ", loss)
        #print(preds)
        #print(F.cross_entropy(input=preds[0], target=y[0]))
    ...
    def run_train_iter(self, data_batch, epoch):
        ...
        question, x_support_set, x_target_set, y_support_set, y_target_set = data_batch

        question = question.long().to(device=self.device)
        x_support_set = x_support_set.float().to(device=self.device)
        x_target_set = x_target_set.float().to(device=self.device)
        y_support_set = y_support_set.long().to(device=self.device)
        y_target_set = y_target_set.long().to(device=self.device)

        data_batch = (question, x_support_set, x_target_set, y_support_set, y_target_set)
